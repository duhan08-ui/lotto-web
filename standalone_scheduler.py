    ...
    try:
        ...
        if config.get('enabled', True):
            ...
            if force_run or auto_condition:
                ...
                # ── 로그 생성 직후 AI 지능형 추천 즉시 실행 ──────────────
                # auto_log가 쌓인 직후 바로 CompositeScore 재계산 → TOP5 갱신
                # LogWatcher도 파일 변경을 감지해 재실행하지만,
                # 여기서 1회 즉시 실행해야 오늘 첫 결과가 바로 반영됨
                # FIX5: success 여부 무관하게 항상 실행
                # (오늘 이미 실행됨으로 skip된 경우에도 Supabase 로그 반영을 위해 재실행)
                logger.info('▶ AI 지능형 추천 즉시 실행')
                try:
                    ...
                    for line in ai_result.splitlines():
                        if any(k in line for k in ('순위:', 'CompositeScore', '누적 로그 반영', '생성일시')):
                            logger.info('  [AI] %s' % line.strip())
                    logger.info('  AI 지능형 추천 갱신 완료')
                    results['ai_intelligent'] = True
                except Exception as e:
                    logger.error('  AI 지능형 추천 실패: %s' % e, exc_info=True)
                    results['ai_intelligent'] = False
            else:
...
def run_daemon():
    ...
    logger.info('  - 실시간: 로그 변경 감지 시 AI 지능형 추천 자동 갱신 (LogWatcher)')
    logger.info('Ctrl+C 로 종료')
    logger.info('=' * 60)

    # ── LogWatcher 시작 (백그라운드 데몬 스레드) ──────────────────────────
    log_watcher = None
    try:
        import importlib.util
        spec = importlib.util.spec_from_file_location('log_watcher', PROJECT_DIR / 'log_watcher.py')
        lw_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(lw_module)

        log_watcher = lw_module.LogWatcher(PROJECT_DIR)
        started = log_watcher.start(daemon=True)
        if started:
            logger.info('[LogWatcher] 실시간 로그 감시 시작 완료')
        else:
            logger.warning('[LogWatcher] 시작 실패 (watchdog 미설치 가능성)')
    except Exception as e:
        logger.warning('[LogWatcher] 로드 실패: %s — 실시간 감시 없이 계속 실행' % e)
    # ─────────────────────────────────────────────────────────────────────
