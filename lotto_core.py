...
class LottoPredictor:
    ...
    def score_manual_combination(self, numbers):
        ...
        # ──────────────────────────────────────────────────────────────────
        # [공동당첨(분배금) 위험도] — 정직하게 재정의된 지표
        #
        # ⚠️ 중요: 로또 1등 당첨 확률은 모든 조합이 1/8,145,060 으로 동일합니다.
        #   이 점수는 "당첨 가능성"과 무관합니다. 과거 데이터·패턴은 다음 추첨을
        #   예측하지 못합니다(각 회차는 독립·균등 무작위).
        #
        # 통계적으로 유일하게 유효한 개입은 "당첨 시 분배금"입니다.
        #   사람들이 많이 고르는 조합(연속수, 생일대 1~31 쏠림, 등차수열 등)은
        #   당첨됐을 때 공동 당첨자가 많아 1인당 수령액이 줄어듭니다.
        #   이런 인기 패턴을 피하면 "만약 당첨될 경우" 독식 가능성이 높아집니다.
        #
        #   · sharing_risk_score : 0(독식 유리) ~ 1(분배 위험 큼)
        #   · sharing_grade      : "낮음" / "보통" / "높음"
        #   이 지표는 당첨 확률이 아니라 "당첨 시 기대 수령액"에만 영향을 줍니다.
        # ──────────────────────────────────────────────────────────────────
        sharing = self._jackpot_sharing_estimate(sorted_numbers)
        ...
        return {
            ...
            # 공동당첨(분배금) 위험도 — 당첨 확률과 무관, 당첨 시 분배 측면만 평가
            "sharing_risk_score": sharing["sharing_risk_score"],
            "sharing_grade":      sharing["sharing_grade"],
            "sharing_reasons":    sharing["reasons"],
        }

    # ──────────────────────────────────────────────────────────────────────
    # [공동당첨(분배금) 위험도] 내부 계산 메서드
    # ──────────────────────────────────────────────────────────────────────
    def _jackpot_sharing_estimate(self, numbers: list) -> dict:
        """조합의 '공동당첨 위험도'를 추정한다.

        ⚠️ 이 메서드는 당첨 확률을 계산하지 않는다. 로또 1등 당첨 확률은
        모든 조합이 1/8,145,060 으로 동일하며, 과거 데이터로 예측할 수 없다.

        대신, 사람들이 통계적으로 더 많이 선택하는 '인기 패턴'을 얼마나
        포함하는지를 평가한다. 인기 패턴이 많을수록 (만약 당첨될 경우)
        공동 당첨자가 많아 1인당 수령액이 줄어든다. 따라서 인기 패턴을
        피하는 것은 '당첨 시 기대 수령액'을 높이는, 통계적으로 유효한 유일한
        개입이다.
        ...
        Returns:
            dict:
                sharing_risk_score - 0.0(독식 유리) ~ 1.0(분배 위험 큼)
                sharing_grade      - "낮음" / "보통" / "높음"
                reasons            - 위험을 높인 패턴 사유 리스트
        """
        nums = sorted(int(n) for n in numbers)
        risk = 0.0
        reasons: list[str] = []

        # ① 생일대(1~31) 쏠림 — 날짜로 번호를 고르는 사람이 매우 많음
        birthday_count = sum(1 for n in nums if n <= 31)
        if birthday_count == 6:
            risk += 0.32
            reasons.append("6개 모두 1~31 (생일/날짜 조합 — 인기 매우 높음)")
        elif birthday_count == 5:
            risk += 0.16
            reasons.append("5개가 1~31 구간 (생일 편중)")

        # ② 연속수 — 2,3,4 같은 연속 배열은 사람들이 즐겨 선택
        longest_run = 1
        current_run = 1
        ...
        if longest_run >= 4:
            risk += 0.26
            reasons.append(f"{longest_run}개 연속수 (인기 패턴)")
        elif longest_run == 3:
            risk += 0.13
            reasons.append("3개 연속수")

        # ③ 등차수열(일정 간격) — 5,10,15,20...
        diffs = [nums[i + 1] - nums[i] for i in range(len(nums) - 1)]
        if len(set(diffs)) == 1:
            risk += 0.22
            reasons.append("완전 등차수열 (규칙적 패턴 — 다수 선택)")

        # ④ 같은 끝자리 반복 — 7,17,27,37 등
        last_digit_counts = Counter(n % 10 for n in nums)
        max_same_tail = max(last_digit_counts.values())
        if max_same_tail >= 3:
            risk += 0.12
            reasons.append(f"같은 끝자리 {max_same_tail}개")

        # ⑤ 5의 배수/10의 배수 쏠림 — 기억하기 쉬워 선택률 높음
        multiples_of_5 = sum(1 for n in nums if n % 5 == 0)
        if multiples_of_5 >= 4:
            risk += 0.10
            reasons.append("5의 배수 4개 이상")

        # ⑥ 홀짝 또는 고저 완전 편중 — 6:0 구성은 시각적으로 선호됨
        odd_count = sum(1 for n in nums if n % 2 == 1)
        if odd_count in (0, 6):
            risk += 0.08
            reasons.append("홀수 또는 짝수로만 구성")
        low_count = sum(1 for n in nums if n <= 22)
        if low_count in (0, 6):
            risk += 0.08
            reasons.append("저구간 또는 고구간으로만 구성")

        sharing_risk_score = round(min(risk, 1.0), 4)
        if sharing_risk_score >= 0.45:
            grade = "높음"
        elif sharing_risk_score >= 0.20:
            grade = "보통"
        else:
            grade = "낮음"
        if not reasons:
            reasons.append("뚜렷한 인기 패턴 없음 — 분배 위험 낮음")

        return {
            "sharing_risk_score": sharing_risk_score,
            "sharing_grade": grade,
            "reasons": reasons,
        }
