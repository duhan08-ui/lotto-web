...
# [성능] 대시보드 데이터 함수는 매 rerun 마다 lotto.xlsx 와 여러 jsonl 로그를
# 다시 읽어 1초 이상 소요됐다. 파일 변경 시그니처(mtime/size)를 캐시 키로
# 사용해, 실제 데이터가 바뀜 때만 재계산하도록 st.cache_data 로 감싼다.
_DASHBOARD_SIG_FILES = (
    "lotto.xlsx",
    "reports/performance_analysis.json",
    "logs/prediction_log.jsonl",
    "logs/probability_log.jsonl",
    "logs/manual_score_log.jsonl",
    "logs/lotto_history.db",
    "logs/weekly_winner_log.jsonl",
    "logs/auto_weekly_result.json",
)


def _dashboard_cache_token(project_dir):
    project_dir = Path(project_dir)
    sig = []
    for rel in _DASHBOARD_SIG_FILES:
        fp = project_dir / rel
        try:
            stt = fp.stat()
            sig.append((rel, stt.st_mtime_ns, stt.st_size))
        except OSError:
            sig.append((rel, 0, 0))
    return tuple(sig)

def ball_color(n):
    """번호대별 숫자 색 (라이트 가독 팔레트). 공은 중립 원, 숫자에만 적용."""
    n = int(n)
...
def _compute_performance_data(project_dir):
    project_dir = Path(project_dir)
...
def _compute_winner_data(project_dir):
    project_dir = Path(project_dir)
    reports_dir = project_dir / 'reports'
...
def _load_pred_records_by_label(project_dir):
    """[수정] 예측 로그를 '로그분석/히스토리 화면과 동일한 소스(DB)'에서 읽는다.
    배포 직후 서버는 Supabase 동기화가 DB(lotto_history.db)만 채우고 jsonl 은 비워두는
    경우가 있어, jsonl 만 읽던 대시보드가 0으로 보였다. DB 우선 → 실패/빈 경우 jsonl 폴백.
    반환: {'패턴추천': [(target_round, numbers)], '확률추천': [...], '수동추천': [...]}"""
    lt_label = {'prediction': '패턴추천', 'probability': '확률추천', 'manual': '수동추천'}
    # 1순위: DB(lotto_history.db) 를 '직접' 읽는다 — 히스토리 화면이 보는 바로 그 파일.
    #   무거운 로드·재동기화 경로를 거치지 않아 부작용이 없고, 매 rerun 에 호출돼도
    #   '됐다 안 됐다' 없이 항상 같은 결과가 나온다(읽기 전용 sqlite).
    out = {'패턴추천': [], '확률추천': [], '수동추천': []}
    try:
        import sqlite3
        db_path = Path(project_dir) / 'logs' / 'lotto_history.db'
        if db_path.exists():
            con = sqlite3.connect('file:%s?mode=ro' % db_path, uri=True)
            try:
                rows = con.execute(
                    "SELECT log_type, target_round, source_round, numbers_json FROM log_records"
                ).fetchall()
            finally:
                con.close()
            got = False
            for lt, tr, sr, nj in rows:
                lb = lt_label.get(lt)
                if not lb:
                    continue
                try:
                    nums = json.loads(nj) if nj else None
                except Exception:
                    nums = None
                if not nums or len(nums) != 6:
                    continue
                t = int(tr) if tr else (int(sr) + 1 if sr else None)
                if not t:
                    continue
                try:
                    out[lb].append((t, [int(n) for n in nums]))
                    got = True
                except Exception:
                    pass
            if got:
        ...
        pass
    # 2순위 폴백: jsonl 직접 읽기 (기존과 동일 동작)
    out = {'패턴추천': [], '확률추천': [], '수동추천': []}
    logs_dir = Path(project_dir) / 'logs'
    files = {'패턴추천': 'prediction_log.jsonl', '확률추천': 'probability_log.jsonl', '수동추천': 'manual_score_log.jsonl'}
    for lb, fn in files.items():
        fp = logs_dir / fn
        if not fp.exists():
            continue
        try:
            with open(fp, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    try:
                        e = json.loads(line)
                        tr = e.get('target_round', e.get('source_round', 0))
                        nums = e.get('numbers')
                        if tr and nums and len(nums) == 6:
                            out[lb].append((int(tr), [int(n) for n in nums]))
                    except Exception:
                        pass
        except Exception:
            pass
...
def _compute_ai_recommendation_hit_data(project_dir):
    """
    ...
    if excel_path.exists():
        try:
            ...
            # [성능] iterrows() 대신 벡터화: 필요한 컴럼만 정수 변환 후 일괄 처리.
            has_bonus = '보너스' in df.columns
            cols = ['회차'] + num_cols + (['보너스'] if has_bonus else [])
            sub = df[cols].apply(pd.to_numeric, errors='coerce').dropna(subset=['회차'])
            for rec in sub.to_dict('records'):
                try:
                    rnd = int(rec['회차'])
                    nums = sorted(int(rec[c]) for c in num_cols)
                    bonus = int(rec['보너스']) if has_bonus and not pd.isna(rec['보너스']) else None
    ...
    if not actual_results:
        # [버그수정] 호출부가 (hits, pred_by_round) 튜플로 언패킹하므로
        # 빈 결과도 동일한 튜플 형태로 반환한다(이전엔 []를 반환해 언패킹 오류 발생).
        return [], {}
    
    # [수정] jsonl 대신 히스토리와 동일 소스(DB 우선)에서 예측 로그를 읽는다.
    by_label = _load_pred_records_by_label(project_dir)

    hits = []
    pred_by_round = {}   # {target_round: 해당 회차 총 예측 조합 수} - 자동+수동 전부 포함

    for log_type_label in ('패턴추천', '확률추천', '수동추천'):
        for target_round, pred_nums in by_label.get(log_type_label, []):
            ...
            if hit_count == 6:
                prize, prize_order = '1등', 1
            elif hit_count == 5 and bonus_match:
                prize, prize_order = '2등', 2
            elif hit_count == 5:
                prize, prize_order = '3등', 3
            elif hit_count == 4:
                prize, prize_order = '4등', 4
            elif hit_count == 3:
                prize, prize_order = '5등', 5
            else:
                continue
            hits.append({
                'log_type': log_type_label, 'target_round': target_round,
                'pred_numbers': sorted(pred_nums), 'actual_numbers': actual_nums,
                'bonus_number': bonus, 'matched_numbers': matched,
                'hit_count': hit_count, 'bonus_match': bonus_match,
                'prize': prize, 'prize_order': prize_order,
                'candidate_rank': '-', 'score': '-', 'timestamp': '', 'run_id': '',
            })
...
def _cached_performance_data(project_dir_str, _token):
    return _compute_performance_data(project_dir_str)


@st.cache_data(show_spinner=False)
def _cached_winner_data(project_dir_str, _token):
    return _compute_winner_data(project_dir_str)


@st.cache_data(show_spinner=False)
def _cached_ai_recommendation_hit_data(project_dir_str, _token):
    return _compute_ai_recommendation_hit_data(project_dir_str)


def get_performance_data(project_dir):
    return _cached_performance_data(str(project_dir), _dashboard_cache_token(project_dir))


def get_winner_data(project_dir):
    return _cached_winner_data(str(project_dir), _dashboard_cache_token(project_dir))


def get_ai_recommendation_hit_data(project_dir):
    return _cached_ai_recommendation_hit_data(str(project_dir), _dashboard_cache_token(project_dir))
...
def render_performance_dashboard(project_dir):
    ...
    <style>
        ...
        /* ── LIGHT EDITORIAL 오버라이드 (대시보드 카드 라이트화) ── */
        .lotto-ball, .ai-hit-ball { background:#F4F1EA !important; border:1px solid rgba(28,25,20,.14) !important; box-shadow:none !important; }
        .card, .ai-hit-card {
            background:#FFFEFB !important;
            border:1px solid rgba(28,25,20,.10) !important;
            box-shadow:0 1px 2px rgba(28,25,20,.04),0 10px 30px -16px rgba(28,25,20,.14) !important;
        }
        .card-header { color:#9A9489 !important; }
        .card-title { color:#201E1A !important; }
        .card-sub { color:#9A9489 !important; }
        .section-header { color:#201E1A !important; border-bottom-color:rgba(28,25,20,.10) !important; }
        .card-value { color:#201E1A; font-family:'Pretendard',sans-serif; font-weight:500; }
        .card-1st, .card-2nd, .card-3rd, .card-4th, .card-5th, .card-top5 { border-left-color:rgba(45,74,107,.45) !important; }
        .card-1st .card-value, .card-2nd .card-value, .card-3rd .card-value,
        .card-4th .card-value, .card-5th .card-value, .card-top5 .card-value { color:#201E1A !important; }
        .progress-container { background:rgba(28,25,20,.06) !important; }
        .progress-red { background:#B0413E !important; }
        .progress-orange { background:#A6772E !important; }
        .progress-yellow { background:#9A7B1F !important; }
        .progress-gold { background:#8A6B2F !important; }
        .progress-green { background:#2F7D5B !important; }
        .progress-blue { background:#2C5A8A !important; }
        .progress-purple { background:#2D4A6B !important; }
        .data-table th { background:rgba(28,25,20,.04) !important; color:#9A9489 !important; }
        .data-table td { color:#201E1A !important; border-bottom-color:rgba(28,25,20,.06) !important; }
        .data-table tr:hover { background:rgba(28,25,20,.03) !important; }
        .hit-1st { color:#B0413E !important; } .hit-2nd { color:#A6772E !important; } .hit-3rd { color:#9A7B1F !important; }
        .hit-4th { color:#6E7B43 !important; } .hit-5th { color:#2F7D5B !important; } .hit-top5 { color:#2C5A8A !important; }
        .ai-hit-card { background:linear-gradient(135deg,rgba(176,65,62,.05),#FFFEFB 60%) !important; border-color:rgba(176,65,62,.26) !important; }
        .ai-hit-card.prize-2 { background:linear-gradient(135deg,rgba(166,119,46,.05),#FFFEFB 60%) !important; border-color:rgba(166,119,46,.26) !important; }
        .ai-hit-card.prize-3 { background:linear-gradient(135deg,rgba(154,123,31,.05),#FFFEFB 60%) !important; border-color:rgba(154,123,31,.26) !important; }
        .ai-hit-card.prize-4 { background:linear-gradient(135deg,rgba(110,123,67,.05),#FFFEFB 60%) !important; border-color:rgba(110,123,67,.24) !important; }
        .ai-hit-card.prize-5 { background:linear-gradient(135deg,rgba(47,125,91,.05),#FFFEFB 60%) !important; border-color:rgba(47,125,91,.24) !important; }
    </style>
    ...
    st.markdown(f'**1·2·3등 달성 현황** <span style="font-size:12px; color: #9A9489;">전체 {total_pred_count}조합 / {pred_total}회차 예측 기준</span>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        <div class=\"card card-1st\">
            ...
            <div class=\"progress-container\">
                <div class=\"progress-bar progress-red\" style=\"width: {min(rate_1st, 100)}%;\"></div>
            </div>
    ...
    with col2:
        ...
        <div class=\"card card-2nd\">
            ...
            <div class=\"progress-container\">
                <div class=\"progress-bar progress-orange\" style=\"width: {min(rate_2nd, 100)}%;\"></div>
            </div>
    ...
    with col3:
        ...
        <div class=\"card card-3rd\">
            ...
            <div class=\"progress-container\">
                <div class=\"progress-bar progress-yellow\" style=\"width: {min(rate_3rd, 100)}%;\"></div>
            </div>
            <div class=\"card-sub\">예측 {total_pred_count}조합 중 {total_3rd}조합 적중</div>
        </div>
        ''', unsafe_allow_html=True)
    
    st.markdown(f'**4·5등 달성 현황** <span style="font-size:12px; color: #9A9489;">전체 {total_pred_count}조합 / {pred_total}회차 예측 기준</span>', unsafe_allow_html=True)
    col1, col2, col3 = st.columns(3)
    ...
    with col1:
        ...
        <div class=\"card card-4th\">
            ...
            <div class=\"progress-container\">
                <div class=\"progress-bar progress-gold\" style=\"width: {min(rate_4th, 100)}%;\"></div>
            </div>
    ...
    with col2:
        ...
        <div class=\"card card-5th\">
            ...
            <div class=\"progress-container\">
                <div class=\"progress-bar progress-green\" style=\"width: {min(rate_5th, 100)}%;\"></div>
            </div>
    ...
    with col3:
        ...
        <div class=\"card card-top5\">
            ...
            <div class=\"progress-container\">
                <div class=\"progress-bar progress-blue\" style=\"width: {min(rate_top5, 100)}%;\"></div>
            </div>
    ...
    if winner:
        ...
        for n in win_nums:
            ...
            balls_html += f'<span class=\"lotto-ball\" style=\"color:{ball_color(n)};\">{n:02d}</span>'
    ...
    # ========================================
    st.markdown(f'<p class=\"section-header\">AI 지능형 추천 번호 당첨 현황 <span style=\"font-size:14px; color: #9A9489;\">전체 {total_pred_count}조합 / {pred_total}회차 예측 기준</span></p>', unsafe_allow_html=True)
    ...
    if True:  # 항상 실행
        ...
        # 요약 카드 (이모지 없이 · 차분한 잉크 숫자)
        summary_cols = st.columns(5)
        for i, prize_name in enumerate(['1등', '2등', '3등', '4등', '5등']):
            cnt = prize_counts.get(prize_name, 0)
            with summary_cols[i]:
                st.markdown(f'''
                <div class=\"card\" style=\"text-align: center; border-left: 3px solid rgba(45,74,107,.45);\">
                    <div style=\"font-size: 12px; color: #9A9489; letter-spacing: 1px;\">{prize_name}</div>
                    <div style=\"font-size: 30px; font-weight: 500; color: #201E1A; font-family: 'Pretendard',sans-serif; margin-top: 6px;\">{cnt}건</div>
                </div>
                ''', unsafe_allow_html=True)
        ...
        # [성능] 이 목록·페이지네이션을 fragment 로 격리 → 페이지 넘김 클릭 시
        # 페이지 전체가 아니라 이 조각만 재실행되어 즉시 갱신된다.
        @st.fragment
        def _ai_hit_cards_fragment():
            # ── 페이지네이션 설정 ──────────────────────────────────────
            ITEMS_PER_PAGE = 3
            ...
            st.markdown(f'''
            <style>
                /* 현재 페이지 번호 버튼만 강조 (화살표 버튼은 제외) */
                div[class*="st-key-ai_page_num_"] button:disabled {{
            ...
            def render_pagination(suffix):
                ...
                with col_next2:
                    if st.button('⏭', key=f'ai_page_last_{suffix}', disabled=(current_page == total_pages), use_container_width=True):
                        ...
                        st.rerun(scope="fragment")
                st.markdown(
                    f'<div style="text-align:center; color: #6A655C; font-size: 12px; margin-top: 2px;">'
            ...
            for hit in page_hits:
                ...
                color, icon = prize_colors.get(prize_name, ('#2D4A6B', ''))
                ...
                for n in hit['pred_numbers']:
                    if n in hit['matched_numbers']:
                        pred_balls += f'<div class=\"ai-hit-ball\" style=\"background:#F4F1EA; color:{ball_color(n)}; border:1px solid rgba(28,25,20,0.14);\">{n:02d}</div>'
                    else:
                        pred_balls += f'<div class=\"ai-hit-ball\" style=\"background:#F4F1EA; color:#9A9489; border:1px solid rgba(28,25,20,0.10);\">{n:02d}</div>'
            
                # 실제 당첨 번호 볼
                actual_balls = ''
                for n in hit['actual_numbers']:
                    if n in hit['matched_numbers']:
                        actual_balls += f'<div class=\"ai-hit-ball\" style=\"background:#F4F1EA; color:{ball_color(n)}; border:1px solid rgba(28,25,20,0.14);\">{n:02d}</div>'
                    else:
                        actual_balls += f'<div class=\"ai-hit-ball\" style=\"background: rgba(45,74,107,0.08); color: #9A9489;\">{n:02d}</div>'
                ...
                <div class=\"ai-hit-card {card_class}\">
                    <div style=\"display: flex; align-items: center; gap: 10px; margin-bottom: 6px;\">
                        <span class=\"ai-hit-prize-badge\" style=\"background: rgba(45,74,107,0.08); color: {color}; border: 1px solid {color};\">{prize_name}</span>
                        <span style=\"color: #2D4A6B; font-size: 13px;\">{hit["target_round"]}회차</span>
                        <span style=\"color: #9A9489; font-size: 12px;\">| {hit["log_type"]}</span>
                    </div>
                    <div style=\"font-size: 12px; color: #6A655C; margin-bottom: 6px;\">예측 번호 (강조 = 일치)</div>
                    <div class=\"ai-hit-balls\">{pred_balls}</div>
                    <div style=\"font-size: 12px; color: #6A655C; margin-bottom: 4px;\">실제 당첨 번호</div>
    ...
    if len(trend_data) >= 2:
        ...
        <div class=\"card\" style=\"text-align: center; padding: 40px;\">
            ...
            <div style=\"color: #9A9489;\">아직 충분한 추이 데이터가 없습니다.</div>
            <div style=\"color: #9A9489; font-size: 12px; margin-top: 10px;\">최소 2회 이상 분석 실행 필요</div>
        </div>
        ''', unsafe_allow_html=True)
