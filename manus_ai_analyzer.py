...
'''
    ...
    def parse_manus_response(self, response: str, default_target_round: int = None) -> dict:
        '''Manus 응답 파싱 - 1~5순위 모두 추출'''
        result = {
            'raw_response': response,
            'recommended_sets': [],
            'success': False,
            'source_round': None,  # Manus 응답에서 추출한 회차 번호
            'target_round': default_target_round
        }
        
        recommended_sets = []
        
        # Manus 응답에서 회차 번호 추출 (예: "1116회차", "1116" 등)
        if not response.startswith('에러:'):
            round_pattern = r'(\d{4})회차'
            match = re.search(round_pattern, response)
            if match:
                result['source_round'] = int(match.group(1))
                logger.info('Manus 응답에서 회차 번호 추출: %d' % result['source_round'])
        
        # 방법 1: "순위:" 형식으로 직접 파싱 (한글 형식)
        rank_pattern = r'(\d+)순위[:\s]+([\d\s,]+)'
        rank_matches = re.finditer(rank_pattern, response)
        for match in rank_matches:
            nums_str = match.group(2)
            numbers = re.findall(r'\b([0-9]{1,2})\b', nums_str)
            if len(numbers) >= 6:
                try:
                    unique_numbers = sorted(set(int(n) for n in numbers[:6] if 1 <= int(n) <= 45))
        ...
        bracket_matches = re.finditer(bracket_pattern, response)
        ...
        # source_round가 추출되지 않았으면 기본값 사용
        if result['source_round'] is None:
            result['source_round'] = default_target_round - 1 if default_target_round else 0
    ...
    def run_analysis(self, custom_instructions: str = None, use_manus: bool = None, save_results: bool = True) -> dict:
        ...
        if use_manus:
            ...
            parsed = self.parse_manus_response(raw_response, target_round)
            result['recommended_sets'] = parsed['recommended_sets']
            result['success'] = parsed['success']
            result['source_round'] = parsed.get('source_round')  # Manus 응답에서 추출한 회차 번호 포함
    ...
    def _save_analysis_results(self, result: dict):
        ...
        # 추천 세트를 수동 로그(manual_score_log.jsonl)에만 저장 + 리포트 파일에도 저장
        if result['success'] and result['recommended_sets']:
            from log_utils import get_round_context
            ctx = get_round_context(self.project_dir / 'lotto.xlsx')
            # Manus 응답에서 추출한 source_round를 우선 사용 (없으면 ctx에서 가져옴)
            source_round = result.get('source_round') or ctx.get('source_round') or 0
            ...
            for i, numbers in enumerate(result['recommended_sets'], 1):
                ...
                # 수동 로그(manual_score_log.jsonl)에만 저장 - Manus AI 번호는 수동 로그 전용
                manual_record = {
                    "timestamp": self.utc_now_iso(),
                ...
                persist_log_record(self.log_dir, "manual", manual_record)
                
                logger.info(f'Manus 분석 결과 수동 로그 저장: {i}순위 - {nums_str}')
        else:
