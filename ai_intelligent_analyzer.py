...
class AIIntelligentAnalyzer:
    ...
    def _build_stats(self, rows):
        ...
        for row in rows[:window]:
            for a, b in combinations(row, 2):
                k = (min(a,b), max(a,b))
                cooc_cnt[k] = cooc_cnt.get(k, 0) + 1
        p_n = {n: freq.get(n, 0.5) / (window * N_DRAW) for n in UNIVERSE}
        cooc_pmi = {}
        for (a, b), cnt in cooc_cnt.items():
            p_ab = cnt / max(window, 1)
            # FIX2: 표준 PMI = log(p_ab / (p_n[a]*p_n[b]))
            # 기존 코드의 *window 는 불필요하며 PMI 전체를 log(window)≈5.3 만큼 축소했음
    ...
    def _s2_cooccur(self, nums, stats):
        ...
        # FIX2: PMI 정규화 기준 7.0 → 9.0
        # 올바른 PMI 이론적 최댓값 ≈ log(window*N_DRAW²) = log(200*36) ≈ 8.88
        # /9.0 으로 정규화하면 0~1 범위 안전하게 보장
        return min(_mean([max(pmi.get((a,b), 0.0), 0.0) for a,b in pairs]) / 9.0, 1.0)
    ...
    def calculate_ac_value(self, nums):
        ...
        연속수처럼 차이가 단조로우면 0에 가깝고, 분산될수록 커진다(0~14).
        """
