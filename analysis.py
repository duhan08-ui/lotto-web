...
def _explode_numbers(df: pd.DataFrame, actual_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in df.to_dict(orient="records"):
        ...
        if target_round is None or int(target_round) not in actual_map:
            continue
        actual_numbers = actual_map[int(target_round)]["actual_numbers"]
        ...
        row["actual_draw_date"] = actual_map[int(target_round)]["draw_date"]
        records.append(row)
...
def _prepare_prediction_df(base_dir: Path, actual_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not matched_df.empty:
        matched_df = matched_df.sort_values(["target_round", "score"], ascending=[False, False]).reset_index(drop=True)
    return matched_df
...
def analyze_logs(base_dir: Path | str, excel_path: Path | str) -> dict[str, Any]:
    ...
    best_example = None
    if not matched_df.empty:
        best_row = matched_df.sort_values(["hit_count", "score"], ascending=[False, False]).iloc[0].to_dict()
        best_example = {
