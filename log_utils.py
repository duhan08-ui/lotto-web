...
def ensure_log_database(log_dir: Path) -> Path:
    ...
    try:
        ...
        if any(keyword in error_msg for keyword in ['disk', 'i/o', 'readonly', 'permission', 'corrupt']):
            if db_path.exists():
                try:
                    db_path.unlink()
                except Exception:
...
def load_combined_log_history(base_dir: Path | str) -> pd.DataFrame:
    ...
    # [BUG FIX] Streamlit은 모듈 상태를 프로세스 메모리에 유지하므로
    # _RUNTIME_REMOTE_BOOTSTRAP_DONE 캐시가 채워지면 이후 Supabase 재fetch가 차단됨.
    # 히스토리 로드 시마다 캐시를 초기화해 GHA가 저장한 최신 데이터를 항상 반영함.
    _RUNTIME_REMOTE_BOOTSTRAP_DONE.discard(str((base_dir / "logs").resolve()))
