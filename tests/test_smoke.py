...
class DefaultStateRegressionTest(unittest.TestCase):
    ...
    def test_missing_state_defaults_to_unlimited_start_and_zero_counts(self):
        ...
        # 저장된 상태가 없으면 '제한 모드'(unlock_granted=False)로 시작한다.
        # 무제한 사용은 UNLOCK_PASSWORD 입력으로만 해제되는 의도된 보안 동작이며,
        # 사용 횟수 카운트는 0에서 시작한다.
        self.assertFalse(state["unlock_granted"])
...
class StreamlitAppSmokeTest(unittest.TestCase):
    ...
    def test_authenticated_dashboard_renders_cleaner_home(self):
        ...
        self.assertIn("이렇게 쓰세요", markdown_blob)
        self.assertIn("숫자 읽는 법", markdown_blob)
    ...
    def test_analysis_view_shows_operating_metrics(self):
        ...
        self.assertIn("누적 추천 건수", markdown_blob)
        self.assertIn("시뮬레이션 규모", markdown_blob)
        self.assertIn("핫 넘버 TOP3", markdown_blob)
        self.assertIn("다음 추첨 D-DAY", markdown_blob)
        self.assertIn("오늘 생성 추천", markdown_blob)
...
class LogDetailTableRegressionTest(unittest.TestCase):
    def test_log_detail_table_shows_all_rows_when_preview_limit_disabled(self):
        ...
        self.assertIn("화면에 표시합니다", caption_text)
...
class LogAnalysisRegressionTest(unittest.TestCase):
    def test_same_second_analysis_runs_keep_all_records(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # [설계 변경] 분석 요약은 더 이상 달력 로그(log_type="analysis")로 저장되지 않는다
            # (MODIFICATIONS_SUMMARY.md 참고). 따라서 히스토리에는 analysis 레코드가 없어야 한다.
            history_df = load_combined_log_history(base_dir)
            if "log_type" in history_df.columns:
                analysis_count = int((history_df["log_type"] == "analysis").sum())
            else:
                analysis_count = 0
            self.assertEqual(analysis_count, 0)

            # 핵심 회귀 가드: 같은 초에 실행해도 run_id는 충돌 없이 고유해야 한다.
...
class UpdatePipelineRegressionTest(unittest.TestCase):
    ...
    def test_weekly_workflow_is_manual_only_and_keeps_update_step(self):
        # [변경] 기존 weekly-lotto-analysis.yml 은 lotto_scheduler.yml 로 통합되었다.
        # 현재 워크플로우는 스케줄(cron)과 수동 실행(workflow_dispatch)을 모두 지원하고,
        # gha_daily.py / gha_saturday.py 를 실행하며 갱신된 lotto.xlsx 를 커밋한다.
        workflow_path = PROJECT_DIR / ".github" / "workflows" / "lotto_scheduler.yml"
        self.assertTrue(workflow_path.exists(), "lotto_scheduler.yml 워크플로우가 없습니다")
        workflow_text = workflow_path.read_text(encoding="utf-8")

        self.assertIn("workflow_dispatch:", workflow_text)
        self.assertIn("schedule:", workflow_text)
        self.assertIn("gha_daily.py", workflow_text)
        self.assertIn("gha_saturday.py", workflow_text)
        self.assertIn("git add lotto.xlsx", workflow_text)
