...
class TestLoadSecretFunction(unittest.TestCase):
    ...
    def test_secrets_toml_file_exists(self):
        """secrets.toml.example 템플릿이 .streamlit 폴더에 존재하는지 확인.

        실제 secrets.toml은 .gitignore로 제외되는 민감정보이므로(저장소에 커밋 금지),
        커밋되어야 하는 안전한 템플릿(.example)의 존재를 검증한다.
        """
        example_path = PROJECT_DIR / ".streamlit" / "secrets.toml.example"
        self.assertTrue(example_path.exists(), ".streamlit/secrets.toml.example 파일이 없습니다")

    def test_secrets_toml_contains_required_keys(self):
        """secrets.toml.example 템플릿에 필수 키가 모두 포함됐는지 확인."""
        example_path = PROJECT_DIR / ".streamlit" / "secrets.toml.example"
        content = example_path.read_text(encoding="utf-8")
        for key in ["SIMULATION_EDIT_PASSWORD", "DATA_CHECK_PASSWORD", "UNLOCK_PASSWORD"]:
            self.assertIn(key, content, f"{key}가 secrets.toml.example에 없습니다")
