...
class PerformanceAnalyzer:
    ...
    def load_history(self, limit=None):
        ...
        [BUG FIX] 기존엔 limit=200 + df.head(200) 이라서 엑셀이 회차 오름차순일 때
        가장 '오래된' 1~200회차만 비교 대상이 됐다. 예측 로그의 target_round 는 전부
        최근 회차(1200번대)이므로 history 와 절대 겹치지 않아 3등·4등·5등 적중이
        항상 0으로 집계됐다(집계 공식 자체는 정상).
        → 기본값을 전체(None)로 바꾸고, limit 가 주어지면 '최신' 회차부터 자른다.
        """
        ...
        has_bonus_col = '보너스' in df.columns

        # 회차 오름차순 정렬 → limit 가 있으면 최신 회차 우선으로 자름
        if has_round_col:
            df = df.copy()
            df['_round_sort'] = pd.to_numeric(df['회차'], errors='coerce')
            df = df.sort_values('_round_sort', na_position='first')
        ...
        for _, row in df.iterrows():
            ...
            if len(nums) == 6:
                ...
                if has_bonus_col:
                    try:
                        bonus_num = int(row['보너스'])
                    except Exception:
                        bonus_num = None
                history.append({'round': round_num, 'numbers': sorted(nums), 'bonus': bonus_num})
        return history
    ...
    def analyze_all_rounds(self, max_rounds=None):  # None = 전체 누적 (제한 없음)
        ...
        for actual in (history if max_rounds is None else history[:max_rounds]):
            ...
            round_result = {
                ...
                'total_predictions': 0,
                'match_6': 0,
                'match_5': 0,        # 5개 일치, 보너스 미일치 = 3등
                'match_5_bonus': 0,  # 5개 일치 + 보너스 = 2등
                'match_4': 0,
                'match_3': 0,
                'match_2': 0,
                'match_1': 0,
            ...
            bonus = actual.get('bonus')  # lotto.xlsx 보너스 번호 (2등 판정용)
            ...
            for entry in entries:
                ...
                if matches == 6:
                    ...
                    # 5개 일치 + 보너스 번호까지 일치 → 2등, 아니면 3등
                    if bonus is not None and int(bonus) in pred_nums:
                        round_result['match_5_bonus'] += 1
                    else:
    ...
    def calculate_performance_metrics(self, results, recent_weeks=20):
        ...
        # match_4 = 4개 일치 → 4등
        # match_3 = 3개 일치 → 5등
        total_2nd = sum(r.get('match_5_bonus', 0) for r in results)  # 5개+보너스 = 2등
        ...
        rounds_with_2nd = len([r for r in results if r.get('match_5_bonus', 0) > 0])
        ...
        recent_2nd = sum(r.get('match_5_bonus', 0) for r in recent_results)
        ...
        recent_rounds_with_2nd = len([r for r in recent_results if r.get('match_5_bonus', 0) > 0])
        ...
        hit_rate_2nd  = (rounds_with_2nd  / max(total_rounds, 1)) * 100
        ...
        recent_hit_rate_2nd  = (recent_rounds_with_2nd  / max(len(recent_results), 1)) * 100
        ...
        metrics = {
            'total': {
                ...
                'hit_rate_2nd':  round(hit_rate_2nd,  1),
            ...
            'recent': {
                ...
                '4th_hits': recent_4th,
                '5th_hits': recent_5th,
                'rounds_with_2nd': recent_rounds_with_2nd,
                ...
                'hit_rate_2nd':  round(recent_hit_rate_2nd,  1),
    ...
    def generate_performance_report(self, metrics, results, patterns):
        ...
        report.append('-' * 75)
        report.append('')
        report.append('  🥈 2등 달성률 (5개+보너스): ' + str(total['hit_rate_2nd']) + '% (' + str(total['rounds_with_2nd']) + '/' + str(total['rounds']) + '회차)')
        ...
        report.append('')
        report.append('  총 2등 당첨 (5개+보너스): ' + str(total['total_2nd_hits']) + '건')
        ...
        report.append('-' * 75)
        report.append('')
        report.append('  🥈 2등 달성률: ' + str(recent['hit_rate_2nd']) + '% (' + str(recent['rounds_with_2nd']) + '/' + str(recent['weeks']) + '회차)')
        ...
        report.append('')
        report.append('  최근 2등: ' + str(recent['2nd_hits']) + '건')
