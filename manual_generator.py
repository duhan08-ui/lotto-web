...
def create_record(log_type: str, rank: int, source_round: int, target_round: int):
    ...
    # ──────────────────────────────────────────────────────────────────
    # [Kelly Criterion] 로그 생성기용 더미 Kelly 값
    # 실제 운영 시에는 LottoPredictor.score_manual_combination() 반환값의
    # kelly_fraction / kelly_recommendation 을 그대로 저장합니다.
    # 여기서는 가우시안 분포로 시뮬레이션된 probability_score를
    # Kelly 공식에 대입하여 더미 값을 생성합니다.
    #
    # Kelly 공식: f* = (b·p - (1-p)) / b,  b=0.45 (로또 실효 순배당률)
    # p는 score를 0~1로 정규화한 값 (score/100 클리핑)
    # ──────────────────────────────────────────────────────────────────
    b = 0.45
    p_norm = max(0.01, min(0.99, score / 100.0))
    q_norm = 1.0 - p_norm
    kelly_fraction = round((b * p_norm - q_norm) / b, 4)
    ...
    return {
        ...
        # Kelly Criterion 필드 (수동 모드에서 실제 계산값으로 대체됨)
