...
def run_ai_analysis(use_manus=False):
    ...
    if use_manus and os.getenv("MANUS_API_KEY"):
        try:
            # Manus 분석 모듈은 PROJECT_DIR 내부의 함수를 직접 호출
    ...
    # 수동 점수 로그에도 저장 (사용자 요청)
    from log_utils import log_manual_score
    for res in pattern_results:
        log_manual_score(
            ...
            numbers=res['sorted'],
            result={
                'best_order': res.get('ordered') if res.get('ordered') is not None else res['sorted'],
                'best_score': res['score'],
                'average_score': res['score'],
                'probability_score': res.get('probability_weight', 0)
            }
        )
