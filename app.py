...
def _render_analysis_view(
    ...
    with tab_top5:
        _render_top5_log_tab(project_dir, excel_path, excel_cache_token)
...
def main():
    ...
    # [BUG FIX] 매 rerun마다 update_excel + cache clear 가 실행되어 메모리 폭발(Bus error) 유발
    # → 세션당 1회만 실행하도록 플래그로 제어
    if not st.session_state.get("_source_data_refreshed"):
        try:
