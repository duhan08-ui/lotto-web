...
def _render_clickable_calendar(history_df: pd.DataFrame, selected_date: date) -> None:
    ...
    # [UX] 오늘 날짜 셀 강조: 키 기반 클래스로 해당 날짜 버튼만 틸 톤으로 표시
    # (날짜 선택 팝업 달력의 '오늘' 스타일과 동일한 톤. 선택(primary) 상태보다 우선하도록
    #  button[kind] 셀렉터를 함께 지정해 특이도를 확보한다.)
    _today_obj = datetime.now(KST).date()
    _today_cell_key = f"history_calendar_{_today_obj.strftime('%Y-%m-%d')}"
    st.markdown(
        f"""
        <style>
        div[class*="st-key-{_today_cell_key}"] button,
        div[class*="st-key-{_today_cell_key}"] button[kind="primary"],
        div[class*="st-key-{_today_cell_key}"] button[kind="secondary"] {{
            background: rgba(78,200,212,.14) !important;
            border: 1.5px solid rgba(52,167,182,.65) !important;
            color: #176B76 !important;
            font-weight: 700 !important;
            box-shadow: 0 0 0 2px rgba(78,200,212,.22) !important;
        }}
        </style>
        """,
        unsafe_allow_html=True,
    ...
    for week in cal.monthdayscalendar(year, month):
        ...
        for idx, day_value in enumerate(week):
            with cols[idx]:
                ...
                st.caption(f"오늘 · {count}건" if day_obj == _today_obj else f"{count}건")
                st.caption(types)
...
def _render_log_date_filter(logs_df: pd.DataFrame, key_prefix: str, current_target_round: int | None = None) -> tuple[pd.DataFrame, str]:
    ...
    _start_str = st.session_state[start_key].strftime('%Y-%m-%d')
    _end_str = st.session_state[end_key].strftime('%Y-%m-%d')
    period_range = f"{_start_str} ~ {_end_str}"
    ...
    if filter_mode == "round_week" and selected_round is not None and selected_label:
        ...
        # [UI] 카드에는 '~' 없이 라벨 아래로 날짜를 세로 정렬해 표시
        return filtered_df, _format_period_panel_value(_start_str, _end_str, label=period_label)

    st.caption(f"선택 기간: {period_range} · 날짜 기준으로 로그를 바로 검색할 수 있습니다.")
    return filtered_df, _format_period_panel_value(_start_str, _end_str)



def _format_period_panel_value(start_str: str, end_str: str, label: str | None = None) -> str:
    """[UI] '조회 기간' 카드 값 포맷: '~' 없이 시작/종료일을 세로로 정렬해 표시한다.
    라벨(예: '이번주 1231회 로그')이 있으면 제목 줄로 두고 날짜 두 줄을 아래에 붙인다."""
    date_line = (
        "<span style='display:block;font-size:1.04rem;font-weight:500;"
        "letter-spacing:0;line-height:1.6;color:#6A655C;'>{}</span>"
    )
    dates_html = date_line.format(start_str) + date_line.format(end_str)
    if label:
        return f"{label}{dates_html}"
    # 날짜만 있는 경우: 시작일을 제목 크기 그대로, 종료일을 바로 아랫줄로
    return f"{start_str}<span style='display:block;line-height:1.3;'>{end_str}</span>"
    ...
    if _needs_initial_remote_sync and st.session_state.get("auth"):
        ...
        try:
            with st.spinner("최신 데이터를 동기화하는 중입니다..."):
                ...
                _maybe_refresh_source_data(excel_path)
            # [FIX] 동기화된 원격 상태를 세션에 재반영한다.
            # 첫 렌더(로컬 우선)에서 구버전 로컬 값으로 시드된 counts/unlock 등이
            # 이후 _persist_runtime_state 호출 시 원격 최신 값을 덮어쓰는 것을 방지.
            # 원격 영속화(Supabase)가 켜진 경우에만 의미가 있으므로 그때만 수행한다.
            if remote_persistence_enabled(project_dir):
                _fresh_state = load_app_state(project_dir)
                st.session_state["counts"] = dict(_fresh_state.get("counts", {"prediction": 0, "probability": 0, "manual": 0}))
                st.session_state["unlock_granted"] = bool(_fresh_state.get("unlock_granted", False))
                st.session_state["data_access_granted"] = bool(_fresh_state.get("data_access_granted", False))
                st.session_state["simulation_count"] = _sanitize_simulation_count(_fresh_state.get("simulation_count", DEFAULT_SIMULATION_COUNT))
