...
def generate_auto_logs(
    ...
    try:
        ...
        # [중복 방지 강화] 트리거가 두 번(예: cron 중복/수동 실행) 울려도 하루 목표치로 고정.
        # gha_daily 는 시작 시 Supabase 로그를 로컬 JSONL 로 내려받으므로, 오늘 로그가
        # 이미 목표치만큼 존재하면 last_run 값과 무관하게 추가 생성을 건너뛴다.
        # (last_run 은 실행 컨테이너마다 공유되지 않을 수 있어, 실제 로그 건수로 판정하는 게 안전)
        if not force_regenerate:
            ...
            if (today_pred >= prediction_count
                    and today_prob >= probability_count
                    and today_manual >= manual_count):
                logger.info(
                    '오늘(%s) 로그가 이미 충분합니다 (패턴 %d/%d, 확률 %d/%d, 수동 %d/%d). 추가 생성을 건너뜁니다.' % (
                        today, today_pred, prediction_count, today_prob, probability_count,
                        today_manual, manual_count
                    )
                )
                return True
