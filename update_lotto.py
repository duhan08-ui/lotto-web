...
REQUIRED_COLUMNS = ["회차", "추첨일", "번호1", "번호2", "번호3", "번호4", "번호5", "번호6", "보너스", "수집페이지", "출처"]
...
def parse_cards(html: str, page_no: int) -> list[dict]:
    ...
    for card in cards:
        ...
        for node in number_nodes:
            ...
            if num_match:
                parsed_numbers.append(int(num_match.group()))
        if len(parsed_numbers) < 7:
            continue

        numbers = parsed_numbers[:6]
        bonus_number = parsed_numbers[6]
...
def save_excel(df: pd.DataFrame, file_path: Path) -> None:
    with pd.ExcelWriter(file_path, engine="openpyxl") as writer:
        ...
        widths = {"A": 10, "B": 14, "C": 8, "D": 8, "E": 8, "F": 8, "G": 8, "H": 8, "I": 8, "J": 12, "K": 55}
